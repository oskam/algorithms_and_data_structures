 ...
 */
public class OSTree<K extends Comparable<K>> {
    ...
    public void delete(K key) {
        ...
        while (node.parent != null) {
            node.lSize = (node.left).lSize + (node.right).lSize + 1;
            node = node.parent;
        }
    }
     ...
     */
    public void inorder(Node<K> x) {
        if (x != null) {
            inorder(x.left);
            System.out.print(x.key + "(" + x.lSize + ") ");
            inorder(x.right);
        } else {}
    }
    
    public void postorder(Node<K> x) {
        if (x != null) {
            postorder(x.right);
    ...
    /**
     * Method finding k-th smallest element in x subtree
     */
    public Node<K> osSelect(Node<K> x, int k) {
        int rank = x.lSize + 1;
        ...
        if (rank == k) {
            ...
            return osSelect(x.right, k - rank);
        }
    }
    
    public Node osSelect(int k) {
        Node<K> x = osSelect(root, k);

        return x;
        //System.out.println("node with rank " + k + " is: " + x.key);
    }
    
    /**
     * Method returning the position of x in linear sorted list of elements of the tree
     */
    public int osRank(Node<K> x) {
        int rank = x.lSize + 1;
        
        Node<K> y = x;
        while (y != root) {
            if (y == y.parent.right)
                rank = rank + y.parent.lSize + 1;
            y = y.parent;
        }
    ...
    public int osRank(K x) {
        Node<K> node = searchNode(x);
        return osRank(node);
        //System.out.println(x + " has a rank of: " + osRank(node));
    }
}
